...
| Phase 6 | **Multi-node (P6.0–P6.5).** Two adapter nodes against one Redis: P6.0 harness, P6.1 cross-node presence (Redis presence manager), P6.2 cross-node revocation (broker feed + sweep) / idempotency (free) / nonce-bound (documented), P6.3 serial-order audit (offset-driven, no rewrite), P6.4 comet pinned single-node (documented), P6.5 two-node validation suite (cross-node delivery / presence / revocation / resume / AIT rewind). WebSocket fully cross-node; comet LB-pinned. fly `count=2` deploy is the remaining Matt-gated step | `TestMultiNode*_P6_*` green under `ABLY_REDIS_TEST=1`; single-node gate still 481/481 both engines; each item code-reviewer-PASS |
