...
| Phase 6 | **Multi-node (P6.0–P6.5).** Two adapter nodes against one Redis: P6.0 harness, P6.1 cross-node presence (Redis presence manager), P6.2 cross-node revocation (broker feed + sweep) / idempotency (free) / nonce-bound (documented), P6.3 serial-order audit (offset-driven, no rewrite), **P6.4(b) cross-node comet via survey forwarding** (the owning node id rides the connectionKey; wrong-node per-key requests forward over the broker control channel — the mechanism centrifuge's own emulation layer uses; no LB pinning needed), P6.5 two-node validation suite (cross-node delivery / presence / revocation / resume / AIT rewind). WebSocket AND comet fully cross-node. **Deployed live at `count=2`** and verified: a message survived a full restart of every machine (Redis durability), a publish forced to machine A was read on machine B (cross-node shared state), and a comet session created on machine A was driven end-to-end — attach, publish, receive, close — through machine B (`fly-force-instance-id`) | `TestMultiNode*_P6_*` green under `ABLY_REDIS_TEST=1`; single-node gate still 481/481 both engines; live durability + cross-node (WS and comet) confirmed on `rt-poc-demo.fly.dev`; each item code-reviewer-PASS |
